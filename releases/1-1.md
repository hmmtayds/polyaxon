 ...
 * **New**: Add handling for preset and patch strategies in the `-f` override behavior.
 * **New**: Add `--git-preset` and `--git-revision` to enable integration and injection of git presets before submitting an operation:
   ...
   * **Note 1**: In order to use this flag, you need to run first `polyaxon init [--git-connection] [--git-url]` and follow the wizard to initialize
     the current workdir with information about the git repo or the git connection to inject automatically in your operation.
   * **Note 3**: Polyaxon CLI will patch the operation with `post_merge` strategy, i.e. it will set/append the git repo initializer to the init definition in the specification.
 ...
 * **Fix**: Regression in compatibility checks introduce with the minimum interval validation, the infinite value `-1` was ignored.
 ...
 * **New**: Add new Fastai v2 callback.
 * **Enhancement**: Patch ignite logger to use new tracking module.
 * **Enhancement**: Add handling for torch tensor.
 ...
 * **Enhancement**: Move quick-start, features and architecture, and concepts to intro section.
 * **Fix**: Several issues.
